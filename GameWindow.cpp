...
void game_init() {
    ...
    ALLEGRO_BITMAP *icon = al_load_bitmap("./image/power.png");
    al_set_display_icon(display, icon);
}

void game_begin() {
    // Load sound
    song = al_load_sample("./sound/beat.wav");
...
int process_event(){
    ...
    if( window == 1 ){
        ...
        boss_process(event);
        minion_process(event);
        minion1_process(event);
        minion2_process(event);
        equip_process(event);
    }
